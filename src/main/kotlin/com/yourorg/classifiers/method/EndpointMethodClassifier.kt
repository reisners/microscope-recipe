...
class EndpointMethodClassifier(model: OntModel) : AbstractMethodClassifier(model) {
    ...
    override fun classify(
        ...
        val requestMappingClassLevel = classAnnotations["RequestMapping"]!!["value"]?.map { any -> any as String }
            ?.toTypedArray()
        ...
        httpMethods.forEach { httpMethod ->
            val individualEndpoint = model.createIndividual(createABoxURI(httpMethod, *requestMappingClassLevel ?: emptyArray(), *requestMappingMethodLevel), model.getOntClass(CLASS_ENDPOINT)).also {
